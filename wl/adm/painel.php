...
if ($_SESSION == ['categoria_usuario' => $_SESSION['categoria_usuario'], 'usuario' => $_SESSION['usuario']]) {
    retorne('desenho', 'desenho');
}
...
if (true) { //usuarios
    ...
    if (isset($_POST['add_usuarios']) || isset($_SESSION['add_usuarios'])) {
        ...
        <td><input type="text" placeholder="Email do novo usuario" name="usuarios_email"></td>
        <td><input id="telefone" maxlength="15" type="text" placeholder="Whazapp do novo usuario" name="usuarios_wazapp"></td>
    ...
    if (isset($_POST['add_usuarios1'])) {
        ...
        if ($_POST['usuario_clase'] != '') {
            if (str_contains($_POST['usuarios_email'], '@')) {
                if (strlen($_POST['usuarios_wazapp']) == 15) {
                    if ($_POST['usuarios_nome'] != '') {
                        if ($_POST['usuarios_senha'] != '') {
                            $result = sql("SELECT * FROM `usuarios` WHERE `nome` = '" . $_POST['usuarios_nome'] . "'");
                            if (!mysqli_num_rows($result)) {
                                sql("INSERT INTO `usuarios`(`whatsapp`, `email`,`individuo`, `data_hora_add`,`nome`,`senha`,`tipo`, `status`)VALUES ('55" . str_replace([') 9', '(', '-'], ['', '', ''], valid($_POST['usuarios_wazapp'])) . "','" . valid($_POST['usuarios_email']) . "','" . nome_id('usuarios', $_SESSION['usuario'], 'nome') . "','" . date('d/m/Y H:i:s') . "','" . valid($_POST['usuarios_nome']) . "',md5('" . valid($_POST['usuarios_senha']) . "'),'" . valid($_POST['usuario_clase']) . "','ativo')");
                                //$_SESSION = ['categoria_usuario' => $_SESSION['categoria_usuario'],'usuario' => $_SESSION['usuario']];
                                reload('painel.php');
                            } else {
    ...
    if (isset($_SESSION['ini_usuarios'])) {
        $array_nome = select_lin('usuarios', 'nome', '`status` != \'adm\' and `nome` != \'' . valid($_SESSION['usuario']) . '\'');
        ...
        if(isset(\$_POST[\'butao_tabela".$l."\'])){
            modal(\'Mudar informação de  ".$valores[$l]."\', \'<p>Se não quiser mudar um campo deixe-lo vazio</p>
            <input placeholder=\"Nova senha do usuario\" class=\"input_text\" type=\"text\" name=\"novo_nome".$l."\" multiple=\"multiple\" autofocus/>
            <input placeholder=\"Novo email do usuario\" class=\"input_text\" type=\"text\" name=\"novo_email".$l."\" multiple=\"multiple\" autofocus/>
            <input id=\"telefone\" maxlength=\"15\" placeholder=\"Nova whazap do usuario \" class=\"input_text\" type=\"text\" name=\"novo_whazapp".$l."\" multiple=\"multiple\" autofocus/>  
            
            \', \'botao_novo_nome".$l."\', \'Mudar\');
        ...
        if(isset(\$_POST[\'botao_novo_nome".$l."\'])){
                       ...
                       if(\$_POST[\'novo_nome".$l."\'] != \"\"){
                        if(strlen(\$_POST[\'novo_nome".$l."\'])>=8){
                        sql(\"INSERT INTO `historico_adm`(`info_mais`,`nome`,`id_adm`, `item`, `status`, `data_add`) VALUES (\'".$valores[$l]."\',\'" . nome_id(\'usuarios\',$valores[$l],\'nome\') . "\',\'' . nome_id('usuarios', $_SESSION['usuario'], 'nome') . '\',\'usuarios\',\'auterou_senha\',\'\".date(\'d/m/Y H:i:s\').\"\')\");
                        sql(\"UPDATE `usuarios` SET `senha`=md5(\'\".\$_POST[\'novo_nome".$l."\'].\"\') WHERE `nome`=\'".$valores[$l]."\'\");
                        }else{
                            alert(\"Senha tem que possuir no minimo 8 caracteres\");
                        }   
                    }
                    if(\$_POST[\'novo_whazapp".$l."\'] != null){
                        if(strlen(\$_POST[\'novo_whazapp".$l."\'])==15){
                            sql(\"INSERT INTO `historico_adm`(`info_mais`,`nome`,`id_adm`, `item`, `status`, `data_add`) VALUES (\'".$valores[$l]."\',\'" . nome_id(\'usuarios\',$valores[$l],\'nome\') . "\',\'' . nome_id('usuarios', $_SESSION['usuario'], 'nome') . '\',\'usuarios\',\'auterou_whazapp\',\'\".date(\'d/m/Y H:i:s\').\"\')\");
                            sql(\"UPDATE `usuarios` SET `whatsapp`=\'55\" . str_replace([\') 9\', \'(\', \'-\'], [\'\', \'\', \'\'], valid(\$_POST[\'novo_whazapp".$l."\'])) . \"\' WHERE `nome`=\'".$valores[$l]."\'\");
                        }else{
                        ...
                        if(\$_POST[\'novo_email".$l."\'] != \"\"){
                            if(str_contains(  \$_POST[\'novo_email".$l."\'],\"@\")){
                                sql(\"INSERT INTO `historico_adm`(`info_mais`,`nome`,`id_adm`, `item`, `status`, `data_add`) VALUES (\'".$valores[$l]."\',\'" . nome_id(\'usuarios\',$valores[$l],\'nome\') . "\',\'' . nome_id('usuarios', $_SESSION['usuario'], 'nome') . '\',\'usuarios\',\'auterou_email\',\'\".date(\'d/m/Y H:i:s\').\"\')\");
                                sql(\"UPDATE `usuarios` SET `email`=\'\".\$_POST[\'novo_nome".$l."\'].\"\' WHERE `nome`=\'".$valores[$l]."\'\");
                            }else{
        ...
        for ($i = 0; $i < count($array_nome); $i++) {
            $tempp = '';
            $array = select_lin('usuarios', 'whatsapp', '`nome` = \'' . $array_nome[$i] . '\'')[0];
            for ($l=0; $l != 14; $l++) { 
                if($l == 0){
                    $tempp .= '+';
                }
                ...
                $tempp .= substr($array,$l,1);
            }
            array_push($array_whazap, $tempp);
        }
        $array_email = [];
        for ($i = 0; $i < count($array_nome); $i++) {
            array_push($array_email, select_lin('usuarios', 'email', '`nome` = \'' . $array_nome[$i] . '\'')[0]);
        }


        $result = tabela($array_nome, [$array_whazap,$array_email,$array_tipo], ['Usuarioss','Whatsapp','Email', 'Tipo'], [], [], $php, $array_nome, 'text_button');
        echo ($result[0]);
        eval($result[1]);
    }
}
...
if (true) { //desenho
    ...
    if (isset($_POST['cortados_desenho']) || isset($_SESSION['cortados_desenho'])) {
        ...
        for ($i = 0; $i < count($array_nome); $i++) {
            ...
            $result .= '<td>' . select('historico_desenhos', 'data_hora_mod', "`id_desenhos` = '" . valid($array_id[$i][0]) . "' ")[0][0] . '</td></tr>';

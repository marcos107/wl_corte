...
{
    //../../../../
    $caminho_base = "Z:hd/lixo/";
    $caminho = str_replace("Z:", "", $caminho);
    ...
    rename("Z:" . $caminho . $nome, $caminho_novo . $nome);
    return $caminho_novo;
...
function nome_id($tabela,$name,$coluna,$where = ''){// nome para id
    if($where == ''){
    return select_lin($tabela,'id', "`".$coluna."` = '".$name."' ")[0];
    }
    return select_lin($tabela,'id', "`".$coluna."` = '".$name."' and ".$where)[0];
}
function id_nome($tabela,$id,$coluna){ // id para nome

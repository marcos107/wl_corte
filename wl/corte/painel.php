...
for ($i = 0; $i < count($array_nome); $i++) {
    ...
    $php .= 'if(isset($_POST[\'cortar' . $i . '\'])){
        ...
        if(!mysqli_num_rows(sql("SELECT * FROM `corte` WHERE `id_desenho` = \'' . $id[$i][0] . '\' and `cortador` = \'' . $_SESSION['usuario'] . '\'"))){
            sql("INSERT INTO `corte`(`id_desenho`, `cortador`, `data_add`, `status`) VALUES (\'' . $id[$i][0] . '\',\'' . $_SESSION['usuario'] . '\',\'".date(\'d/m/Y H:i:s\')."\',\'inicio\')");
        }else{
    ...
    sql("INSERT INTO `corte`(`id_desenho`, `cortador`, `data_add`, `status`) VALUES (\'' . $id[$i][0] . '\',\'' . $_SESSION['usuario'] . '\',\'".date(\'d/m/Y H:i:s\')."\',\'finalizado\')");
    sql("INSERT INTO `historico_desenhos`(`status`,`id_desenhos`, `nome`, `data_hora_add`, `data_hora_mod`,`individuo`) VALUES (\'$status => cortado\',\'$id\',\'' . mysqli_real_escape_string($conexao, $array_nome[$i][0]) . '\',\'$data_hora\',\'".date(\'d/m/Y H:i:s\')."\',\'' . $_SESSION['usuario'] . '\')");
    sql("UPDATE `desenhos` SET `status` = \'cortado\',`cortador` = \'' . $_SESSION['usuario'] . '\' WHERE `nome` = \'' . mysqli_real_escape_string($conexao, $array_nome[$i][0]) . '\' and `caminho` = \'' . mysqli_real_escape_string($conexao, $array_caminho[$i][0]) . '\';");
    reload("painel.php");   
}
     ';
}

        ...
        for ($i=0; $i < count($arquivos_pasta); $i++) { 
            ...
            if (!file_exists($abre_dir .  $arquivos_diretorio[$i])) {
                if (in_array($arquivoNovo[sizeof($arquivoNovo) - 1],$filtro)) {
                    ...
                    sql("INSERT INTO `violacao`( `individuo`, `causa`, `data`) VALUES ('".$_SESSION['usuario']."','Tentou upar um arquivo não permitido, arquivo(".substr($arquivos_pasta[$i], strrpos(substr($arquivos_pasta[$i], 0, -1), '\\') + 1, strlen($arquivos_pasta[$i]))."), local(".$abre_dir.")','".date('d/m/Y H:i:s')."')");
...
if ((isset($_POST['file_plus2']) && (isset($_SESSION['arquivo_name'])))) {
    ...
    for ($i = 0; $i < count($arquivo); $i++) {
        if(htmlentities($_POST['prioridade'.$i], ENT_QUOTES, "UTF-8")!='Nada'){
            if(htmlentities($_POST['finalidade'.$i], ENT_QUOTES, "UTF-8")!='Nada'){
            if(htmlentities($_POST['empreendimento'.$i], ENT_QUOTES, "UTF-8")!='Nada'){
            if(htmlentities($_POST['empresa'.$i], ENT_QUOTES, "UTF-8")!='Nada'){
           ...
           $result = mysqli_query($conexao, $query);
            ...
            if(!is_dir($abre_dir.$caminho)){
                mkdir($abre_dir.$caminho, 0700,true);
            }
...
if(isset($_POST['apagar_pasta'])){
   ...
   if($arquivo){
    ...
    rmdir($abre_dir);
    header('Location: dir.php?dir=' . $back_dir); 
   }
